...
function tmp()
	...
	local oauth_consumer_key = "zFtb7cIpfgkknFlf" -- MKM "App Token"
	if public then
		local oauth_token = "" -- public resource: the oauth_token parameter in the Authorization header is empty 
	else
		local oauth_token = "fdYLsAa0yUEEkxvHiKABovSy397bt22S" -- MKM "Access Token"
	end
	local oauth_signature_method = "HMAC-SHA1"
	
	local oauth_timestamp = tostring(os.time())
	--local oauth_nonce = Crypto.hmac.digest("sha1", tostring(math.random()) .. "random" .. tostring(os.time()) , "keyyyy")
	local oauth_nonce = OAuth.generate_nonce()
	
	local baseString = "GET&" .. OAuth.oauth_encode(requestURL) .. "&"
	local paramString = "oauth_consumer_key=" .. OAuth.oauth_encode(oauth_consumer_key) .. "&"
					..	"oauth_nonce=" .. OAuth.oauth_encode(oauth_nonce) .. "&"
					..	"oauth_signature_method=" .. OAuth.oauth_encode(oauth_signature_method) .. "&"
					..	"oauth_timestamp=" .. OAuth.oauth_encode(oauth_timestamp) .. "&"
					..	"oauth_token" .. OAuth.oauth_encode(oauth_token) .."&"
					..	"oauth_version" .. OAuth.oauth_encode(oauth_version)
	paramString = OAuth.oauth_encode(paramString)
	baseString = baseString .. paramString
	
	local mkmAppSecret = "qu1kSFihbGmz6fSveNWmKVQ5KDmCj4J8" -- MKM "App Secret"
	local mkmAccessTokenSecret = "qsWAYhLaOPLlwm2MixBhplCvXa5Zsmci" -- MKM "Access Token Secret"
	
	local signingKey = OAuth.oauth_encode(mkmAppSecret) .. "&"
	if public then else
		signingKey = signingKey .. OAuth.oauth_encode(mkmAccessTokenSecret)
	end
	Games = {"https://www.mkmapi.eu/ws/v1.1/games", method="GET"}
	Expansions = {"https://www.mkmapi.eu/ws/v1.1/expansion/:idGame", method="GET"} -- idGame (MtG) = 1
	ProductsInExpansion = {"https://www.mkmapi.eu/ws/v1.1/expansion/:idGame/:name", method="GET"}
...
function OAuthTest()
	...
	local oauth_consumer_key = "zFtb7cIpfgkknFlf" -- MKM "App Token"
	local consumer_secret = "qu1kSFihbGmz6fSveNWmKVQ5KDmCj4J8" -- MKM "App Secret", first half of signing key
	--local endpoints = { RequestToken = "https://www.mkmapi.eu/ws/authenticate", AuthorizeUser = {"NIL", method = "GET"}, AccessToken = "https://www.mkmapi.eu/ws/v1.1/access" }
	local client = OAuth.new(oauth_consumer_key, consumer_secret, {} )
	
	-- Access Token and Access Token Secret are not needed for widget operation
	local oauth_token = "fdYLsAa0yUEEkxvHiKABovSy397bt22S" -- MKM "Access Token"
	client.SetToken( oauth_token )
	local mkmAccessTokenSecret = "qsWAYhLaOPLlwm2MixBhplCvXa5Zsmci" -- MKM "Access Token Secret"
	client.SetTokenSecret(mkmAccessTokenSecret)
	
	print("Games")
	print(LHpi.Tostring(client.BuildRequest( "GET", "https://www.mkmapi.eu/ws/v1.1/games" )))
	print("Expansions/1")
	print(LHpi.Tostring(client.BuildRequest( "GET", "https://www.mkmapi.eu/ws/v1.1/expansion/1" )))
	print("Expansions with arguments")
	print(LHpi.Tostring(client.BuildRequest( "GET", "https://www.mkmapi.eu/ws/v1.1/expansion" , { idGame = 1 } )))
