--*- coding: utf-8 -*-
--[[- LHpi sandbox breaker
tries to break otut of the MA lua sandbox
...
scriptname = "LHpi.sandboxbreaker-v" .. libver .. "." .. dataver .. "." .. scriptver .. ".lua"
...
function ListStandardLibs()
	...
	log("\tbase")
	for n,f in pairs(basefunctions) do
		log(n ..":".. tostring(f) )
	end
	log("\tcoroutine")
	LHpi.Logtable(coroutine,"coroutine")
	log("\tpackage")
	log("require" .. ":" .. tostring(require))
	LHpi.Logtable(package,"package")
	log("\tstring")
	LHpi.Logtable(string,"string")
	log("\ttable")
	LHpi.Logtable(table,"table")
	log("\tmath")
	LHpi.Logtable(math,"math")
	log("\tbit32")
	LHpi.Logtable(bit32,"bit32")
	log("\tio")
	LHpi.Logtable(io,"io")
	log("\tos")
	LHpi.Logtable(os,"os")
	log("\tdebug")
	LHpi.Logtable(debug,"debug")
end
...
function site.Initialize()
	log,ts= LHpi.Log,LHpi.Tostring
	log "test"
	--ListStandardLibs()
		
	log("trying to loadfile dll")
	local loaddll = loadfile("lua52.dll")
	log(tostring(loaddll))
	
	log("trying to dofile dll")
	local dodll = dofile("lua52.dll")
	log(tostring(dodll))
...
function site.BuildUrl( setid,langid,frucid )
	error("site.BuildUrl not implemeted")
	return {}
...
function site.ParseHtmlData( foundstring , urldetails )
	error("site.ParseHtmlData not implemeted")
	return {}
...
site.sets = {
-- Core Sets
[808]={id = 808, lang = { [1]=true }, fruc = { true }, url = "seturl"},
} -- end table site.sets
