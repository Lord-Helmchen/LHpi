...
defaults to medium prices instead of low
]]
...
function ImportPrice( importfoil , importlangs , importsets )
	...
	--LHpi.Log( "LHpi lib is ready to use." )--true, but logfile name is not known yet :)
	LHpi.DoImport (importfoil , importlangs , importsets, loglater)
...
function site.ReadFilenameOptions(scriptFilename, filenameOptions)
	...
	elseif fnopts["LOW"] then
		...
		himelo = 3
	elseif fnopts["HIGH"] then
		LHpi.Log("\"HIGH\" Filename Option recognized.")
		himelo = 1	
	end--if fnopts himelo
